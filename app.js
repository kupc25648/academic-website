(() => {
  ...
  const FALLBACK_ROUTES = {
    ...
    "/publications/ai-in-architecture/": { view: "publications", publicationPreset: "ai-in-architecture" },
  ...
  const PUBLICATION_PRESETS = {
    "ai-in-architecture": {
      label: "AI in Architecture",
      title: "AI in Architecture Research",
      match: (p) => {
        const text = `${p.title} ${(p.topics || []).join(" ")}`;
        return /AI in Architecture|Computational Architecture|architectural representation|computer vision|machine learning/i.test(text) &&
          /architecture|architectural|design|building|structural/i.test(text);
      }
    },
      ...
      <div class="detail-grid">
        ...
        <div class="detail-content">
          ...
          <div class="detail-actions">
            <a class="detail-link" href="/graduate-research/" data-internal-route>Study or research in this area →</a>
  ...
  function renderGraduateResearch() {
    ...
    const exampleQuestions = data.researchArcs.map((arc, i) => {
      const question = (arc.questions || [])[0];
      if (!question) return "";
      return `<li><a href="${RESEARCH_PATHS[i]}" data-internal-route>${esc(question)}</a></li>`;
    }).join("");
    ...
    target.innerHTML = `
      <div class="detail-content">
        <p class="course-description">
          I supervise graduate research in artificial intelligence and computational methods for architecture, particularly projects involving structural design, optimization, machine learning, reinforcement learning and data-driven design at Chulalongkorn University. This page is for prospective graduate students, research collaborators and visiting researchers. It describes current research directions and learning pathways; it does not imply that a funded position is currently open.
        </p>

        <div class="detail-section">
          <h4>Who this research may fit</h4>
          <ul>
            <li>Architecture students interested in AI beyond image generation, including prediction, reasoning, optimization and decision support.</li>
            <li>Students who want to connect architectural design with structural systems, computation, data and performance.</li>
            <li>Researchers interested in methods such as Python, Grasshopper, machine learning, graph neural networks, reinforcement learning and optimization.</li>
          </ul>
        </div>
        ...
        <div class="tag-row">
          <span class="tag">AI in Architecture</span>
          <span class="tag">Computational Design</span>
          <span class="tag">Structural Intelligence</span>
          <span class="tag">Structural Optimization</span>
          <span class="tag">Machine Learning</span>
          <span class="tag">Reinforcement Learning</span>
        </div>

        <a class="detail-link" href="mailto:${esc(data.profile.email)}">Discuss graduate research ↗</a>
      </div>
    `;
  }
  ...
  function structuredData() {
    ...
    if (profileScript) {
      profileScript.textContent = JSON.stringify({
        ...
        "@id": `${SITE_URL}/#profile`,
        url: `${SITE_URL}/`,
        name: `${p.name} — AI in Architecture & Structural Intelligence`,
        mainEntity: {
          "@type": "Person",
          "@id": `${SITE_URL}/#person`,
          name: p.name,
          alternateName: "ชิตาทร คุปต์วิวัฒน์",
          url: `${SITE_URL}/`,
          jobTitle: p.position,
          email: p.email,
          description: p.about,
